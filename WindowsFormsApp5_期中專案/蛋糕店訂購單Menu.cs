...
namespace WindowsFormsApp5_期中專案
{
    public partial class 蛋糕店訂購單Menu : Form
    {
        ...
        private void 蛋糕店訂購單Menu_Load(object sender, EventArgs e)
        {
            // 設定資料庫連線。
        ...
        void 讀取資料庫()
        {
            ...
            con.Open();
            string strSQL = "select * from Cakestore_products;"; // 連線到資料庫，取出想要的欄位。
        ...
        private void btn訂購_Click(object sender, EventArgs e)
        {
            if ((txt訂購人.Text != "") && (所選個數 > 0)) // 一定要有訂購人，杯數不能為零。
            {
                ...
                SqlCommand cmd = new SqlCommand(strSQL, con);
                cmd.Parameters.AddWithValue("@NewProductName", 所選蛋糕品項); // 將資料注入資料庫中
                ...
                string str口味集合1 = "";
                foreach (string item in list所選口味1)
                {
                    str口味集合1 = str口味集合1 + "," + item;
                }
                cmd.Parameters.AddWithValue("@NewTaste1", str口味集合1);
